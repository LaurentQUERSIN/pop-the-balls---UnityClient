...
namespace Stormancer
{
    ...
    public class Scene : IScene
    {
        ...
        public void AddRoute(string route, Action<Packet<IScenePeer>> handler, Dictionary<string, string> metadata = null)
        {
            if (route[0] == '@')
            {
				this.GetComponent<ILogger>().Error("AddRoute failed: Tried to create a route with the @ character");
            ...
            {
				this.GetComponent<ILogger>().Trace("Created route with id : '{1}'", route);
        ...
        public Task Connect()
        {
            return this._client.ConnectToScene(this, this._token, this._localRoutesMap.Values)
                .Then(() =>
                {
					this.GetComponent<ILogger>().Error("Successfully connected to scene : '{1}'.", Id);
                    this.Connected = true;
                });
        }
